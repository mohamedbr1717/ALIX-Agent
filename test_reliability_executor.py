...
class TestExecutorReliability(unittest.TestCase):
    ...
    def test_git_failure_then_recovery(self):
        with mock.patch(
            "core.executor.subprocess.Popen",
    ...
    @mock.patch("core.executor.subprocess.Popen", side_effect=PermissionError)
    def test_subprocess_permission_error_is_recovered(self, mock_popen):
    ...
    @mock.patch("core.executor.subprocess.Popen", side_effect=FileNotFoundError)
    def test_subprocess_file_not_found_is_recovered(self, mock_popen):
        result = self.executor.run_command("echo should-not-start")
        self.assertFalse(result["ok"])
