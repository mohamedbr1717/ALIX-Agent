...
class ALIXv41Agent:
    ...
    def execute_task(self, task_code, context_query=None):
        """
        NOTE: despite the name, this method never runs `task_code`.
        It only returns a static APPROVED/BLOCKED verdict from the
        AST screen in policy_watcher.py. That screen is a denylist
        heuristic, not a sandbox (see policy_watcher.inspect_code's
        docstring) -- so even an "APPROVED" result must never be
        passed to exec()/eval() without real process isolation.
        """
